...
(* Custom messages for problems importing or invalid settings *)
tmaDataImport::empty = "`1`";
documentProcessingLevel::invalidParameter = "`1`";
...
parseNbContent[Cell[CellGroupData[l_List, ___], ___]] := If[$documentProcessingLevel == "Full", "\\colordiamond{green}", ""] <> parseNbContent[l]
...
convertToLatexDoc[notebookPath_, OptionsPattern[]] :=  Module[{nb, content, latexPath, latexTemplatePath, 
   resourceDir = $resDir, texResult, sownData, filledContent, closeFlag = False, documentProcessingLevel},
  ...
  (* Retrieve, validate and set document processing level *)
  documentProcessingLevel = OptionValue[DocumentProcessingLevel];
  If[documentContentProcessingLevel =!= "", (* Default for fns and pkg is empty string *)
...
convertToLatexAndPdfDocs[notebookPath_, OptionsPattern[]] :=  Module[{latexPath, pdfPath, compileCmd, conversionResult},
  conversionResult = convertToLatexDoc[notebookPath, DocumentProcessingLevel->OptionValue[DocumentProcessingLevel]];
